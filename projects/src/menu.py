...
class Menu:
    ...
    def render(self):
        ...
        # Define la ruta del skeleton
        skeleton_path = 'app/data/ficheros/skltn-graph.gif'

        # Añade un try-except para manejar la posible excepción
        try:
            st.image(f'{IMAGES_DIR}/image_animation.gif',
                     caption='Animación del algoritmo buscando.')
        except FileNotFoundError:
            st.image(skeleton_path, caption='Skeleton')

        # Asegúrate de hacer lo mismo para la otra imagen
        try:
    ...
    def tour_trip(self, origen: int, destino: int):
        ...
        # Ejecutar el algoritmo de recorrido de turismo
        self.aService.turistic_tsp(origen, destino)
        ...
        # Crear el GIF del proceso del algoritmo BFS
