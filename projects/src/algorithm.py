...
class Algorithm:
    ...
    def store_plot_graph(self, step: int, is_road: bool = False):
        ...
        ox.plot_graph(
            self._G,
            node_size=[self._G.nodes[node]["size"] for node in self._G.nodes],
            edge_color=[self._G.edges[edge]["color"] for edge in self._G.edges],
            edge_alpha=[self._G.edges[edge]["alpha"] for edge in self._G.edges],
            edge_linewidth=[self._G.edges[edge]["linewidth"] for edge in self._G.edges],
            node_color=[self._G.nodes[node]["color"] for node in self._G.nodes],
            bgcolor="#18080e",
            ax=ax,
        )
        ...
        for node, data in self._G.nodes(data=True):
            if "tiempo" in data:
                ax.annotate(
                    self._G.nodes[node]["tiempo"], (data["x"], data["y"]), color="white"
                )
            else:
        ...
        plt.savefig(os.path.join(save_dir, f"image_{step}.png"), facecolor="black")
        plt.close(_)
    ...
    def a_star_shortest_path(self, origen, destino):
        ...
        while pq:
            ...
            for edge in self._G.out_edges(node):
                ...
                neighbor = edge[1]
                tentative_g_score = self._G.nodes[node]["g_score"] + self.euc_distance(
                    node, neighbor
                )
                if tentative_g_score < self._G.nodes[neighbor]["g_score"]:
                    ...
                    self._G.nodes[neighbor]["f_score"] = (
                        tentative_g_score + self.euc_distance(neighbor, destino)
                    )
                    heapq.heappush(pq, (self._G.nodes[neighbor]["f_score"], neighbor))
    ...
    def a_star_traffic_lights(self, origen: int, destino: int):
        ...
        while pq:
            ...
            for edge in self._G.out_edges(node):
                ...
                travel_time += self._G.edges[(node, neighbor, 0)].get(
                    "traffic_light_wait", 0
                )
                tentative_g_score += self._G.edges[(node, neighbor, 0)].get(
                    "traffic_light_wait", 0
                )
    ...
    def dijkstra_less_fuel_path(
        ...
        while pq:
            ...
            for edge in self._G.out_edges(node, keys=True):
                ...
                fuel_consumption = edge_length / (max_speed * fuel_efficiency)
                total_fuel_consumed += fuel_consumption
                tentative_g_score = self._G.nodes[node]["g_score"] + fuel_consumption
                if tentative_g_score < self._G.nodes[neighbor]["g_score"]:
                    self._G.nodes[neighbor]["previous"] = node
                    self._G.nodes[neighbor]["g_score"] = tentative_g_score
                    heapq.heappush(pq, (self._G.nodes[neighbor]["g_score"], neighbor))
